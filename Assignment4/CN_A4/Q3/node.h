...
class Node {
 ...
 public:
  ...
  void modifyInterface(string ip){
    for(int i = 0; i < interfaces.size(); i++){
      if(interfaces[i].first.getConnectedIp() == ip){
        interfaces[i].first.setConnectedip("none");
      }
    }
  }
  ...
  void updateTblEntry(string ip, int cost){
    for(int i = 0; i < mytbl.tbl.size(); i++){
      if(mytbl.tbl[i].dstip == ip){
  ...
  void sendMsg(){
    ...
    for (int i = 0; i < interfaces.size(); ++i) {
      if(interfaces[i].first.getConnectedIp() == "none"){
        continue;
      }
      else{
        ...
        for (int j = 0; j < ntbl.tbl.size(); j++){
          if(ntbl.tbl[j].nexthop == interfaces[i].first.getConnectedIp()){
            ntbl.tbl[j].share_cost = 16;
          }
          else{
            ntbl.tbl[j].share_cost = mytbl.tbl[j].cost;
          }
        }
        RouteMsg msg;
